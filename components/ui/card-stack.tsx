...
export function CardStack<T extends CardStackItem>({
  ...
  const [actualWidth, setActualWidth] = React.useState(cardWidth);
  ...
  React.useEffect(() => {
    ...
    const handleResize = () => {
      setActualWidth(Math.min(cardWidth, window.innerWidth - 32));
    };
    handleResize();
  ...
  const cardSpacing = Math.max(10, Math.round(actualWidth * (1 - overlap)));
          ...
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={`cs-card ${isActive ? "cs-card--active" : "cs-card--inactive"}`}
                  style={{
                    width: actualWidth,
                    height: cardHeight,
                    zIndex,
            ...
            {items.map((it, idx) => {
              ...
              return (
                <button
                  ...
                  className={`cs-dot ${on ? "cs-dot--active" : "cs-dot--inactive"}`}
    ...
    <div className="cs-card-default">
      <div className="cs-card-default__img-layer">
        {item.imageSrc ? (
          ...
          <div className="cs-card-default__no-image">No image</div>
        )}
      </div>

      <div className="cs-card-default__overlay" />

      <div className="cs-card-default__content">
        <div className="cs-card-default__title">{item.title}</div>
        {item.description ? (
          <div className="cs-card-default__desc">{item.description}</div>
        ) : null}
      </div>
    </div>
  );
